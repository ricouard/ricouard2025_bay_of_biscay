...
parameter_names <-paste0("q", 0:9)


# capture en nombre par group_isis (de 0 à ...) pour une annee
obs_catch <- readRDS("InputsMACCO/obs_catchKg_megrim_groupisis2017.rds") %>%
 	select(group, obs)


planExperiences <- fread("out_Q/out_Q_Lepidorhombus08/LHS_q.csv", data.table = FALSE, col.names = parameter_names)

	  N_SIM <-	list.files(path = "out_Q/out_Q_Lepidorhombus08", pattern = "row") %>%
	length()
...
if (RERUN_SIMUL) invisible(file.copy(
  from = sprintf("out_Q/out_Q_Lepidorhombus08/row_%04d.csv", 1:N_SIM),
...
save_file_name <- function(suffix) paste0("res_rds/res_rds_lepidorhombus2017/", suffix, ".rds")

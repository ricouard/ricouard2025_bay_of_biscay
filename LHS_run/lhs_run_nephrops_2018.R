...
parameter_names <-paste0("q", 0:57)


# capture en nombre par group_isis (de 0 à ...) pour une annee
obs_catch <- readRDS("InputsMACCO/obs_catchN_nephrops_groupisis2018.rds") %>%
 	select(group, obs)


planExperiences <- fread("out_Q/out_Q_Nephrops2018/LHS_q.csv", data.table = FALSE, col.names = parameter_names)

	  N_SIM <-	list.files(path = "out_Q/out_Q_Nephrops2018", pattern = "row") %>%
	length()
...
if (RERUN_SIMUL) invisible(file.copy(
  from = sprintf("out_Q/out_Q_Nephrops2018/row_%04d.csv", 1:N_SIM),
...
tmp <- foreach(i = 1:N_SIM) %dopar% {
	...
	sim_catch <- file.path(dossier_simul, "CapturesNombre.csv") %>%
...
save_file_name <- function(suffix) paste0("res_rds/res_rds_nephrops2018/", suffix, ".rds")

...
parameter_names <-paste0("q", 0:71)


# capture en nombre par group_isis (de 0 à ...) pour une annee
obs_catch <- readRDS("InputsMACCO/obs_catchKg_hake_groupisis2015.rds") %>%
 	select(group, obs)


planExperiences <- fread("out_Q/out_Q_Merluccius09/LHS_q.csv", data.table = FALSE, col.names = parameter_names)

	  N_SIM <-	list.files(path = "out_Q/out_Q_Merluccius09", pattern = "row") %>%
	length()
...
if (RERUN_SIMUL) invisible(file.copy(
  from = sprintf("out_Q/out_Q_Merluccius09/row_%04d.csv", 1:N_SIM),
...
save_file_name <- function(suffix) paste0("res_rds/res_rds_merluccius2015/", suffix, ".rds")

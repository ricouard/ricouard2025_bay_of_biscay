...
obs_catch <- readRDS("InputsMACCO/obs_catchN_nephrops_groupisis2017.rds") %>%
 	select(group, obs)


planExperiences <- fread("out_Q/out_Q_Nephrops2017/LHS_q.csv", data.table = FALSE, col.names = parameter_names)

	  N_SIM <-	list.files(path = "out_Q/out_Q_Nephrops2017", pattern = "row") %>%
	length()
...
if (RERUN_SIMUL) invisible(file.copy(
  from = sprintf("out_Q/out_Q_Nephrops2017/row_%04d.csv", 1:N_SIM),
...
save_file_name <- function(suffix) paste0("res_rds/res_rds_nephrops2017/", suffix, ".rds")

---
title: "ISIS-Fish MACCO - Model calibration step 1"
...
This document contains the code along with the results of the skill assessment of step 1 model. Simulations are run one year by one year, starting in 2015, 2016, 2017 and 2018, with forced recruitment. This script is also part of the calibration process given that it computes coefficients to perform metier correction as well as seasonalisation of accessibility.
...
In the first section, "Preparation", he will find all the settings and preliminary steps required to perform the analysis, including loading of the companion script and packages, path specifications and control of data importation. In the fourth section, he will find commands for exportation of model correction files to prepare the next validation step.
...
```{r}
sim_2015_path <- paste0(here(),"/Sim_data/raw/step1_TF0q1/sim_calib_mod1_2015_2025-07-08-22-40")
sim_2016_path <- paste0(here(),"/Sim_data/raw/step1_TF0q1/sim_calib_mod1_2016_2025-07-08-22-46")
sim_2017_path <- paste0(here(),"/Sim_data/raw/step1_TF0q1/sim_calib_mod1_2017_2025-07-08-23-09")
sim_2018_path <- paste0(here(),"/Sim_data/raw/step1_TF0q1/sim_calib_mod1_2018_2025-07-08-23-03")
```

The path to tidy simulation data:

```{r}
sim_tidy_path <- paste0(here(),"/Sim_data/tidy/step1_TF0q1/byyear_2015_2018")
```
...
exportedTF_path <- paste0(here(),"/TargetFactors/mod_2b")
```

Wether or not exporting seasonnal accessibility corrections (analysis by season), and in that case, the path to previous accessibility directory as well as the path to exported accessibility files:

```{r}
export_corrections_q <- T
previous_q_path <- paste0(here(),"/accessibility_values/q1_LHS_outputs")
exported_q_path <- paste0(here(),"/accessibility_values/q2_season_step2a")
```
...
Let us do the same for season accessibility:

```{r,message=FALSE}
if(export_corrections_q){
  q_season_corrections <- rbind(Solea_ratio_land_byseason,
                                LepiWhi_ratio_land_byseason,
                                Lophius_ratio_land_byseason,
                                Merluccius_ratio_land_byseason,
                                Nephrops_ratio_land_byseason,
                                Raja_ratio_land_byseason,
                                Leucoraja_ratio_land_byseason) 
  export_new_q_csv(q_season_corrections,previous_q_path,exported_q_path) 
  export_new_q_semantics(q_season_corrections,previous_q_path,exported_q_path) 
}

```

Let us display the new mean accessibility curves by species:

```{r}
for(i in seq(length(Species))){
  this_species <- Species[i]
  if(this_species != "Leucoraja_naevus"){
    print(plot_mean_access(species = this_species,
                           access_path = exported_q_path))
  }
}
